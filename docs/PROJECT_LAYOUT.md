...
- 協調各 stage 的執行順序
- 包含 Stage 1.5（pre-evaluation）與 Stage 6（final evaluation）
- 把 CLI/entry 與 runner 實作分離
- 把 request/runtime context、observability、驗證、統計邏輯拆到鄰近模組
- dashboard 後端/API、模板與前端資產統一放在 `pipeline/` 範圍
...
### `backups/` 與 `docs/archive/`

放歷史快照、研究紀錄與封存文件，不與主線 runtime 混放。

目前建議：

- `backups/`：歷史快照與比對資料
- `docs/archive/`：遷移契約、規劃草案、歷史研究筆記

這些內容屬於封存/研究路徑，不是 `chief.py` 主線執行依賴。
