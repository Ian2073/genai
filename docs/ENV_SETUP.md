...
## 1-2. Ubuntu/Linux 首選流程（建議）

1. 在專案根目錄執行 `bash Build_GenAI.sh`。
	- 腳本會自動偵測 Python 3.11。
	- 腳本會檢查 `g++/clang++`，缺少時可嘗試自動安裝 `build-essential`（Ubuntu）。
2. 首次使用建議先賦予執行權限：`chmod +x Build_GenAI.sh` 與 `chmod +x Start_GenAI.sh`。
3. 此腳本會沿用 `scripts/setup_env.py` 的 GPU profile 安裝策略。
4. 安裝完成後可用以下方式啟動：

```bash
bash Start_GenAI.sh --terminal
bash Start_GenAI.sh --dashboard
...
- `Build_GenAI.sh`：Ubuntu/Linux 本地環境一鍵建置 + 診斷
...
- `Start_GenAI.sh --terminal`：Ubuntu/Linux 終端模式啟動
- `Start_GenAI.sh --dashboard`：Ubuntu/Linux 儀表板模式啟動
- `Start_GenAI.sh --eval-only ...`：Ubuntu/Linux 評測入口
