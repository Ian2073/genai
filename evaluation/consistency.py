...
class AIAnalyzer:
    ...
    def analyze_consistency(self, story_text: str, entities: List[str], kg_info: Dict) -> Dict:
        ...
        try:
            ...
            # 獲取AI主觀評分
            ai_score = self._get_ai_subjective_score(entities, kg_info, self.use_multiple_prompts)
            ai_score = normalize_score_0_100(ai_score, 0.0)
            ...
            # 獲取AI文本分析
            analysis_text = self._get_ai_analysis(unique_entities, entity_counts)
            
            return {
                "analysis": analysis_text,
                "ai_score": ai_score,           # AI主觀評分
                "objective_score": objective_score,  # 客觀統計評分
                "confidence": confidence,       # 混合置信度
    ...
    def _get_ai_subjective_score(self, entities: List[str], kg_info: Dict = None, use_multiple_prompts: bool = False) -> float:
        ...
        if use_multiple_prompts:
            return self._get_ai_score_with_multiple_prompts(entities)
        else:
            return self._get_ai_score_single_prompt(entities)
