...
if modules_start != -1:
    ...
    <section class=\\"tab-panel\\" id=\\"tab_playground\\">
      ...
      <div class=\\"grid-2\\" style=\\"align-items:start;\\">
        <div>
          <section class=\\"card section\\">
            <h3>Standard Chat (通用文本生成)</h3>
            <label>System Prompt</label>
            <input type=\\"text\\" placeholder=\\"You are an expert...\\" style=\\"margin-bottom:8px;\\"/>
            <label>User Message</label>
            <textarea rows=\\"3\\" placeholder=\\"詢問任何問題或撰寫文章...\\"></textarea>
            <button class=\\"btn primary mini\\" style=\\"margin-top:8px;\\">Send Text</button>
            <div style=\\"margin-top:8px; padding:8px; background:#f3f4f6; border-radius:4px; font-size:12px; min-height:60px;\\">...等待中...</div>
          </section>

          <section class=\\"card section\\">
            <h3>Translation Tool (通用翻譯)</h3>
            <div class=\\"grid-2\\" style=\\"margin-bottom:8px;\\">
               <select><option>AutoDetect</option><option>English</option><option>zh-TW</option></select>
               <select><option>zh-TW</option><option>English</option><option>Japanese</option></select>
            </div>
            <textarea rows=\\"3\\" placeholder=\\"貼上想翻譯的文字...\\"></textarea>
            <button class=\\"btn primary mini\\" style=\\"margin-top:8px;\\">Translate</button>
            <div style=\\"margin-top:8px; padding:8px; background:#f3f4f6; border-radius:4px; font-size:12px; min-height:40px;\\">...</div>
          </section>
        </div>

        <div>
          <section class=\\"card section\\">
            <h3>Image Generation (標準影像生成)</h3>
            <label>正面提示詞 (Prompt)</label>
            <textarea rows=\\"2\\" placeholder=\\"A stunning futuristic city...\\"></textarea>
            <label style=\\"margin-top:8px;\\">負面提示詞 (Negative)</label>
            <textarea rows=\\"1\\" placeholder=\\"bad quality, disfigured...\\"></textarea>
            ...
            <button class=\\"btn primary mini\\" style=\\"margin-top:8px;\\">Generate Independent Image</button>
          </section>

          <section class=\\"card section\\">
            <h3>Voice Studio (聲音與錄音)</h3>
            <label>麥克風錄音 (Record Voice)</label>
            <div style=\\"display:flex; gap:8px; margin-bottom:12px; align-items:center;\\">
              ...
              <span id=\\"record_status\\" style=\\"font-size:11px; color:var(--muted);\\">等待錄音...</span>
            </div>
            <label>語音合成 (TTS)</label>
            <textarea rows=\\"2\\" placeholder=\\"輸入想轉成語音的文字...\\"></textarea>
            <div class=\\"grid-2\\" style=\\"margin-top:8px;\\">
               <div><label>Voice Reference</label><select><option>錄製的聲音</option><option>系統預設男聲</option><option>系統預設女聲</option></select></div>
               <div><label>Speed</label><input type=\\"number\\" step=\\"0.1\\" value=\\"1.0\\" /></div>
            </div>
...
'''
html = html.replace('// Elements', js_inj + '\\n    // Elements')

# Fix tab behavior: the existing tab logic assigns events to [data-tab]. I should ensure everything works properly by allowing any valid IDs.
html = html.replace('const allowed = [\\'overview\\', \\'ops\\', \\'detail\\', \\'modules\\'];', 
                    'const allowed = [\\'overview\\', \\'ops\\', \\'detail\\', \\'workbench\\', \\'playground\\'];')
