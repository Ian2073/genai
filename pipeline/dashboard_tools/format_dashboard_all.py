...
'''
# Find the exact JS injection point
js_target = 'const allowed = [\\'overview\\', \\'ops\\', \\'detail\\', \\'modules\\'];'
if js_target in html:
    html = html.replace(js_target, js_inj + '\\n    const allowed = [\\'overview\\', \\'workbench\\', \\'playground\\', \\'ops\\', \\'detail\\'];')
else:
    # If not found (maybe single quotes inside the string?)
    js_target2 = 'const allowed = ["overview", "ops", "detail", "modules"];'
    if js_target2 in html:
        html = html.replace(js_target2, js_inj + '\\n    const allowed = ["overview", "workbench", "playground", "ops", "detail"];')
